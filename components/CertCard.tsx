  ...
  return (<Link href={props.link?props.link:""} passHref >
    ...
    className="group antialiased flex flex-col rounded-lg items-center space-y-7 flex-shrink-0 flex-grow-0 sm:w-96 p-10 md:p-2 bg-[#292929] w-fit  max-h-[80vh] min-h-[75vh] cursor-default ">
      ...
      <div className="px-0 md:px-10 w-[100%] space-y-2 transition ease-in-out duration-300">
        {/*Certification name */}
        <h4 className="text-4xl tracking-wider sm:text-3xl font-light cursor-default pb-2">{props.name}</h4>
        <hr/>
        {/* offered by */}
        <p className="uppercase  text-gray-400 text-base font-medium pt-2">
          Offered By: <span className="text-slate-200">{props.offeredBy}</span>
        </p>
        {/* Provider */}
        ...
        <p className={`uppercase text-gray-400 text-base font-medium pb-2 `}>
          Status: <span className={`text-${props.status=="Completed"||props.status=="completed"?"slate-200":"[#F7AB0A]"}`}>{props.status}</span>
        </p>
        <hr/>
        {/* summary points */}
        <div className="flex flex-wrap">
          {learnPoints.map((item:any,i:number)=>{return <div key={i} className="border py-2 px-3 rounded-3xl my-2 mx-1 tracking-wider hover:scale-105 transition ease-in-out duration-100 font-light hover:border-[#F7AB0A] hover:text-[#F7AB0A] hover:drop-shadow-[0_0px_20px_#ffffff2f] "><p >{item}</p></div>})} 
        </div>
